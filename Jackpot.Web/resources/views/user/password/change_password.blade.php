...
<div class="mt-200">
    <div class="p-2 pt-6 sm:ml-64 mt-14 h-screen">
        ...
        <section class="w-full ">
            ...
            @if(session('success'))
            <div class="bg-green-500 text-white p-4 rounded-md shadow-md mb-4" role="alert">
                <p class="font-bold">{{ session('success') }}</p>
            </div>
            @endif

            @if ($errors->any())
            <div class="bg-red-500 text-white p-4 rounded-md shadow-md mb-4" role="alert">
                <ul>
                    @foreach ($errors->all() as $error)
                    <li class="text-sm mb-2">{{ $error }}</li>
                    @endforeach
                </ul>
            </div>
            @endif
            ...
            <form method="POST" action="/update-password">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" id="large-input" name="old_password"
                ...
                <div class="mb-6">
                    ...
                    <input type="text" id="default-input" name="password"
                        class="block w-full md:w-1/2 p-3 text-white border bg-transparent border-bg-jblue2 rounded-lg text-sm  focus:border-jblue2 focus:ring-jblue2">
                </div>
                <div>
                    <label for="small-input" class="block mb-2 text-sm font-medium text-white">Confirm New Password</label>
                    <input type="text" id="small-input" name="password_confirmation"
                        class="block w-full md:w-1/2 p-3 text-white border bg-transparent border-bg-jblue2 rounded-lg text-sm  focus:border-jblue2 focus:ring-jblue2">
                </div>
                <button type="submit" class="bg-[#00ADB5] text-[#EEEEEE] shadow-[3px_3px_0px_0_rgba(3,119,124,1)] flex items-center gap-2 p-3 rounded-md hover:text-jwhite2  mt-4 focus:ring-4 focus:outline-none focus:ring-jblue2 text-sm w-[200px] px-5 py-2.5 text-center">Update</button>
            </form>
        </section>
    </div>
</div>
